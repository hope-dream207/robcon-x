...
{    
    ...
    if (ecpid->mode == ECPID_POSITION) {
        ...
        // 输出饱和抗积分饱和(Anti-Windup)
        // 当输出已达限幅且误差方向与饱和方向相同时，停止积分累积
        uint8_t windup_blocked = 0;
        if ((ecpid->out >= ecpid->max_out && ecpid->error[0] > 0.0f) ||
            (ecpid->out <= -ecpid->max_out && ecpid->error[0] < 0.0f)) {
            windup_blocked = 1;
        }
        ...
        } else if (!windup_blocked) {
            // 误差过大时缓慢释放积分，减少跃迁反冲。
            ecpid->Iout *= 0.80f;
        }
        // windup_blocked 时积分保持当前值不变
