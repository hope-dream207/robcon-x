...
#define KP_HEADING_MAX  2.3f  // 静止时Kp
#define KP_HEADING_MIN  2.5f   // 全速时Kp
...
void imu_task(void *pvParameters) {
  ...
  for (;;) {
    ...
    {
      float fz = -cmd_z * DEG2RAD;
      taskENTER_CRITICAL();
      chassis_cmd_speed.f_z = fz;
      taskEXIT_CRITICAL();
    }
    vTaskDelay(pdMS_TO_TICKS(2));
 }
...
static void update_odometry(void) {
    ...
    float delta_local_y = delta_mecanum_y * 1.0f;
 ...
 */
 ...
 /* ======== 位置环参数宏 ======== */
#define POS_KP_X        2.2f    /* X轴P */
#define POS_KP_Y        2.0f    /* Y轴P */
#define POS_KD_X        0.01f   /* X轴D */
#define POS_KD_Y        0.005f  /* Y轴D */
#define POS_KI_X        0.0f
#define POS_KI_Y        0.0f
#define POS_VX_MAX      0.15f
#define POS_VY_MAX      0.1f
...
#define POS_W_MAX       1.0f    /* 角速度限幅 rad/s */
...
//                advance_navigation_target(&target_x, &target_y,0.0f, -0.5f);
			          // advance_navigation_target(&target_x, &target_y,0.0f, 1.2f);two
                // advance_navigation_target(&target_x, &target_y,0.0f, 0.78f);
                advance_navigation_target(&target_x, &target_y,0.0f, 0.0f);
              change1_reached_ms = 0U;
              change2_reached_ms = 0U;
              third_entry_ms = HAL_GetTick(); 
              (void)position_loop_update(target_x, target_y, nav_yaw_rad,&cmd_vx, &cmd_vy, &cmd_w,(uint8_t)(order != prev_order));
            ...
            case first: {//前进,3.5//-0.043,0.52,//1618//1625
               cmd_w = 0.0f;
               cmd_vx = 0.0f;
               taskENTER_CRITICAL();
               ...
               taskEXIT_CRITICAL();
               float raw_dist_cm = (float)snap_value0 - 1435.5f; 
//			   float raw_dist_cm = (float)snap_value0 - 1625.5f;
               if(fabsf(snap_value0) >= 3000.0f) {
               cmd_vy = 0.0f;
               third_last_err = 0.0f;
               ...
               cmd_vx = 0.0f;
               cmd_vy = clampf(-(POS_KP_Y * third_filt_dist / 100.0f + POS_KD_Y * third_filt_d / 100.0f)/10.0f, -0.2f, 0.2f);
               if (fabsf(third_filt_dist) <= 3.0f) {
                   cmd_vy = 0.0f;
               }
               }
               if ((fabsf(third_filt_dist) <= 4.0f)||HAL_GetTick() - third_entry_ms >= 30000U) {
                   third_first_tick = 1;
                    advance_navigation_target(&target_x, &target_y, 0.0f, 0.0f);
                    arm_how = stop_3;
                    grip_run = grip_put;
          ...
          case second: {
            ...
            uint8_t reached = position_loop_update(target_x, target_y, nav_yaw_rad, &cmd_vx, &cmd_vy, &cmd_w, sec_pid_reset);
              ...
              cmd_vy = 0.0f;
              if (reached & 1) {
                  if (change1_reached_ms == 0U) {
                      change1_reached_ms = HAL_GetTick();
                  }
                  uint32_t elapsed = HAL_GetTick() - change1_reached_ms;
                  if (elapsed >= 1000U) {
                      grip_run = grip_close;
                  }
                  if (elapsed >= 3000U) {
                      lift_run = lift_up;
                      change2_reached_ms = HAL_GetTick();
                  }
                  if (elapsed >= 5000U) {
            ...
            uint8_t reached = position_loop_update(target_x, target_y, nav_yaw_rad, &cmd_vx, &cmd_vy, &cmd_w, sec_pid_reset);
             ...
             else if(change == 4) {  //前进,-3.0+6.0,7.1-3.6
            uint8_t reached = position_loop_update(target_x, target_y, nav_yaw_rad, &cmd_vx, &cmd_vy, &cmd_w, sec_pid_reset);
              cmd_w = 0.0f;
              cmd_vy = 0.0f;
              cmd_vx = 0.0f;
             ...
             //-7.9,9.78,-11.28,u
              ...
              else if (change == 5) {//0.532;1.01
                  if ((y_run == grip_put_) || ((HAL_GetTick() - change2_reached_ms) >= 85000U)) {
                cmd_w = 0.0f;
                cmd_vy = 0.0f; 
                cmd_vx = 0.0f;
                  grip_run = grip_put;
