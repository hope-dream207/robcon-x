...
void modify_data(MotorCmd_t *motor_s)
{
    ...
    motor_s->hex_len = sizeof(RIS_ControlData_t);
    motor_s->tar_pos = motor_s->Pos;
    motor_s->tar_w = motor_s->W;
    motor_s->tar_t = motor_s->T;
}

/// @brief 将接收到的电机原始数据转换为浮点物理量
...
void extract_data(MotorData_t *motor_r)
{
    ...
    else
    {
        ...
        motor_r->correct = 1;
        motor_r->hex_len = sizeof(RIS_MotorData_t);
        return;
    }
}

void MotorData_calculate_error(MotorData_t *motor_r, float target_pos, float target_vel, float target_torque)
{
    if (!motor_r) return;
    motor_r->pos_error = target_pos - motor_r->Pos;
    motor_r->vel_error = target_vel - motor_r->W;
    motor_r->torque_error = target_torque - motor_r->T;
}

void MotorData_init(MotorData_t *motor_r)
{
    if (!motor_r) return;
    memset(motor_r, 0, sizeof(MotorData_t));
}
