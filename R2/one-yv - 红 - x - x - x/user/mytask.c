          ...
          case second: {
           ...
           case fourth: {//1.5,//5.8(y,-0.4+-0.129)
               ...
               taskEXIT_CRITICAL();
                   ...
                   if (snap_y_run == three_put) {
                   arm_how = arm_put;
                   arm_updown = arm_finish;
                   }
                   fourth_entry_ms = HAL_GetTick();
                   change = 1;}
                // else if (change == 1 && (reached & 1) && ((snap_y_run == three_put) && (HAL_GetTick() - fourth_entry_ms) >= 20000U)) {
                //    advance_navigation_target(&target_x, &target_y, -4.0f, 0.0f);
                //    arm_how = get_put;
                //    fourth_entry_ms = HAL_GetTick();
                //    change = 1;
//                else if (change == 1 && (reached & 1) && ((snap_y_run == three_x) || (HAL_GetTick() - fourth_entry_ms) >= 20000U)) {
			    else if (change == 1 && (reached & 1) && ((snap_y_run == three_x) || (HAL_GetTick() - fourth_entry_ms) >= 5000U)) {
                   advance_navigation_target(&target_x, &target_y, -4.0f, 0.0f);
                   arm_how = stop_3;     
                   arm_updown = up;//打开了气泵
                   fourth_entry_ms = HAL_GetTick();
