/** 
 ...
 //左右反了
 // 上电软启动门控：收到电机反馈后开始计时，保持若干周期不出力，等IMU/CAN收敛
...
//      M_WHEEL_Rr + 0.0068f, // 轮2: 后左 
 ...
 void wheel_inverse_calc(void) { 
   ...
   wheel.wheel_Ow[1] =
      (-f_speed.f_Ovx - (-f_speed.f_Ovw - f_z_val) * w_OL) / (OMIN_WHEEL_Rr+0.05f)/ SQRT2 ; 
 ...
 void wheel_forward_calc(void) { 
   ...
   static const float O_WHEEL_Rr_calib[2] = { OMIN_WHEEL_Rr, OMIN_WHEEL_Rr  };
   for (uint8_t t = 0; t < 2; t++) { 
 ...
 void wheel_calc(void) { 
   // 上电软启动门控：收到电机反馈后开始计时，前 STARTUP_HOLD_MS 周期冻结所有状态，直接发0
