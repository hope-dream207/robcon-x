...
//**************************arm**************************写3区了//
void Cylinder_ControlArm(void)
{
    if (arm_updown == arm_finish || y_run == three_put) {
      HAL_GPIO_WritePin(GPIOB, GPIO_PIN_5, GPIO_PIN_RESET); // 断开
    } else if (arm_how == getx_L || arm_how == getx_H) {
      HAL_GPIO_WritePin(GPIOB, GPIO_PIN_5, GPIO_PIN_SET);   // 吸取
    } else {
      HAL_GPIO_WritePin(GPIOB, GPIO_PIN_5, GPIO_PIN_SET);   // 默认吸取
    }
}