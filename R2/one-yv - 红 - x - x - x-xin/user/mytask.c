    ...
    for (;;) {
       ...
       if (!nav_yaw_locked) {
        ...
        ROS_DataUnpack();
        {
          ...
          //          fourth_entry_ms = HAL_GetTick();
          //          change = 1;}
                // else if (change == 1 && (HAL_GetTick() - fourth_entry_ms) < 3000U) {
                //    arm_updown = arm_finish;
                //    cmd_vx = 0.0f;
                //    cmd_vy = 0.0f;
                //    cmd_w  = 0.0f;
                 if (change == 0 && (reached & 1) ) {
                   advance_navigation_target(&target_x, &target_y, 0.2f, 0.0f);
                   arm_how = get_put;
                   fourth_entry_ms = HAL_GetTick();
                   change = 1;}
                  else if (change == 1 &&(snap_y_run == three_put || (HAL_GetTick() - fourth_entry_ms) >= 6000U)) {
                   arm_updown = arm_finish;
                   cmd_vx = 0.0f;
                   cmd_vy = 0.0f;
                   cmd_w  = 0.0f;
                   change = 2;
               } else if (change == 2 && (HAL_GetTick() - fourth_entry_ms) >= 3000U) {
                   advance_navigation_target(&target_x, &target_y, -2.0f, 0.0f);
                  //  arm_how = arm_put;
                   arm_updown = up;//打开了气泵
                   fourth_entry_ms = HAL_GetTick();
                   change = 3;
               } else if (change == 3 && (reached & 1) && (HAL_GetTick() - fourth_entry_ms) >= 50000U) {
                   order = fifth;
                   change = 0;
