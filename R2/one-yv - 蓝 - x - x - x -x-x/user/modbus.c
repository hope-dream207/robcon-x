/**
  ...
  * @details
  *   架构:
  *     1. CRC16: 256 项预计算查找表, 多项式 0x8005, 初始值 0xFFFF
  *     2. 发送: HAL_UART_Transmit() 阻塞发送, 发送后 HAL_Delay(1) 等待 RS-485 方向切换
  *     3. 接收: DMA 环形缓冲 + UART 空闲中断 (IDLE) 检测帧结束
  *     4. 超时: HAL_GetTick() 轮询, 超时时间 MODBUS_TIMEOUT_MS
  *
  *   硬件约束:
 *     - 严禁操作 RE/DE GPIO (RS-485 模块为硬件自动流向控制)
 *     - USART3_RX 绑定到 DMA1_Stream1_Channel4 (环形模式)
  */
